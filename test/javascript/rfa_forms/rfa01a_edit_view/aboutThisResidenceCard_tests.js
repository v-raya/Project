...
describe('Verify Physical Address', () => {
  ...
  it('verify directions to home change', () => {
    const relationShipField = residenceCardComp.find('#directions')
  ...
  describe('#YesNoRadioComponent', () => {
    ...
    it('verify body of water exists change', () => {
      const relationShipField = residenceCardComp.find('#body_of_water_existtrue').hostNodes()
    ...
    it('verify weapons address change', () => {
      const relationShipField = residenceCardComp.find('#weaponstrue').hostNodes()
      relationShipField.simulate('change', {target: {value: 'false'}})
      expect(setParentStateSpy).toHaveBeenCalledWith('weapon_in_home', 'false')
    })
    it('verify other using residence as mailing change', () => {
      const relationShipField = residenceCardComp.find('#others_using_residence_as_mailingtrue').hostNodes()
      relationShipField.simulate('change', {target: {value: 'false'}})
  ...
  it('verify body of water description', () => {
    const relationShipField = residenceCardComp.find('#body_of_water_description')
  ...
  it('verify residence types address change', () => {
    const relationShipField = residenceCardComp.find('#residenceTypes')
  ...
  it('verify languages select change', () => {
    const relationShipField = residenceCardComp.find('.languages')
...
describe('Verify Physical Address prefix, suffix, first name, middle name and last name', () => {
  ...
  it('verify prefix change', () => {
    const relationShipField = residenceCardComp.find(
  ...
  it('verify adding another persons click', () => {
    const addAnotherPersonButton = residenceCardComp.find('button.btn')
    ...
    const personsList = blankAboutThisResidenceFields.other_people_using_residence_as_mailing
  ...
  it('verify removing person information', () => {
    const removeLink = residenceCardComp.find('.remove-btn').at(1)
    expect(AboutThisResidenceCard.prototype.removeCard).not.toHaveBeenCalled()
    removeLink.simulate('click')
    const personsList = blankAboutThisResidenceFields.other_people_using_residence_as_mailing.slice(1)
