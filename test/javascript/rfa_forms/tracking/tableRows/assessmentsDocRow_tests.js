...
describe('Tracking Show mode', () => {
  ...
  it('edit text area', () => {
    const textAreaField = editAssessmentDocRow.at(0).shallow().find('#assessmentEditText0')
    textAreaField.simulate('change', {target: {value: 'text input'}})
    expect(changeSpy).toHaveBeenCalledWith('notes', 'text input', 0)
  })
  it('edit checkbox', () => {
    const checkBoxField = editAssessmentDocRow.at(0).shallow().find('#assessmentEditCheckbox0')
    checkBoxField.simulate('change', {target: {checked: true}})
    expect(changeSpy).toHaveBeenCalledWith('checked', true, 0)
  })
  it('edit date field', () => {
    const textAreaField = editAssessmentDocRow.at(0).shallow().find('#assessmentEditApprovedDate0')
    textAreaField.simulate('change', {target: {value: '01/20/1980'}})
    expect(changeSpy).toHaveBeenCalledWith('approved_date', '1980-01-20', 0)
  })
  it('edit date field', () => {
    const textAreaField = editAssessmentDocRow.at(0).shallow().find('#assessmentEditSubmittedDate0')
  ...
  it('show text area', () => {
    const textAreaField = showAssessmentDocRow.at(0).shallow().find('#assessmentShowText0')
    expect(textAreaField.text()).toEqual('testing')
  })
  it('show checkbox', () => {
    const checkBoxField = showAssessmentDocRow.at(0).shallow().find('#assessmentShowCheckbox0')
    expect(checkBoxField.length).toEqual(1)
  })
  it('show date field', () => {
    const approvedDateField = showAssessmentDocRow.at(0).shallow().find('#assessmentShowApprovedDate0')
    expect(approvedDateField.text()).toEqual('01/21/1978')
  })
  it('show date field', () => {
    const submittedDateField = showAssessmentDocRow.at(0).shallow().find('#assessmentShowSubmittedDate0')
