...
describe('RFA 02 Out of State Registry check card test', () => {
  ...
  it('handleRegistryClear', () => {
    let outOfStateRegistryCleared0trueField = outOfStateRegistryCheckWithProps.find('#outOfStateRegistryCleared00true').hostNodes()
  ...
  it('handleRegistryClear change Date Field', () => {
    let outOfStateRegistrydate0Field = outOfStateRegistryCheckWithProps.find('#outOfStateRegistrydate0').hostNodes()
    outOfStateRegistrydate0Field.simulate('change', {target: {value: '12/07/2018'}})
    expect(handleRegistryClearSpy).toHaveBeenCalledWith('date', '2018-12-07', 0)
  })

  it('handleRegistryClear change TextArea Field', () => {
    let outOfStateRegistrynotes0Field = outOfStateRegistryCheckWithProps.find('#outOfStateRegistrynotes0').hostNodes()
    outOfStateRegistrynotes0Field.simulate('change', {target: {value: 'Text Area'}})
    expect(handleRegistryClearSpy).toHaveBeenCalledWith('notes', 'Text Area', 0)
  })

  it('handleRequestInfo method with checkbox change', () => {
    let outOfStateRegistry0checkboxEdit0Field = outOfStateRegistryCheckWithProps.find('#outOfStateRegistry0checkboxEdit0').hostNodes()
  ...
  it('handleOutOfStateRegistry', () => {
    let outOfStateRegistry0trueField = outOfStateRegistryCheckWithProps.find('#outOfStateRegistry00true').hostNodes()
